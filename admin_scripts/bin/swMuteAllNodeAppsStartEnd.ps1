<#
    Script : swMuteAllNodeAppsStartEnd.ps1
	...
	DESC : This script will suppress all alerting for node applications from a CSV supplied list within SolarWinds Orion and make use of a supplied start and end time for the alert suppression.
#>
param 
(
...
ForEach ($line in $myCSV) 
{
    ...
    ForEach-Object { Invoke-SwisVerb -SwisConnection $swConn "Orion.AlertSuppression" SuppressAlerts @($entityUris, 
	    [DateTime]::UtcNow.AddMinutes($outageStartInMinutes), [DateTime]::UtcNow.AddMinutes($outageEndInMinutes)) | Out-Null}
}
