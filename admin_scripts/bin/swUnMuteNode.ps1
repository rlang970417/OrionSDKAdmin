<#
	...
	DESC : This script will resume all alerting for nodes from a CSV supplied list within SolarWinds Orion
#>
param 
(
...
ForEach ($line in $myCSV) 
{
    ...
    {
		Invoke-SwisVerb -SwisConnection $swConn -EntityName Orion.AlertSuppression -Verb ResumeAlerts -Arguments @( @($Node.EntityUri), $startArg )
    }
}
