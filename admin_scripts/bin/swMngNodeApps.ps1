<#
	...
	DESC : This script will put all assigned node applications from a CSV supplied list into a managed state within SolarWinds Orion
...
ForEach ($line in $myCSV) 
{
    ...
    {
		<# Manage All Node Apps #>
        Invoke-SwisVerb -SwisConnection $swConn -EntityName Orion.APM.Application -Verb Remanage -Arguments @( $netObjectId[$i] )  | Out-Null
    }
}
